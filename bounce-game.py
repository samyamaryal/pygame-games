...
class Ball:
    def __init__(self, radius, center_x, center_y, velocity):
        self.radius = radius
...
ball = Ball(ball_radius, ball_x, ball_y, ball_velocity)
...
while running:
    ...
    if int(ball.get_yposition() - ball_radius) < 0:
        y_direction = 'down'
    ...
    if int(ball.get_yposition()) == int(bat.get_yposition()) \
        ...
        score += 1
    elif int(ball.get_yposition()) == 0 \
            and int(ball.get_yposition() - ball_radius) >= 0:
        y_direction = 'down' # MOVE DOWN
    ...
    ball.vertical_move(y_direction)
    ball.horizontal_move(x_direction)
